...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
                            ...
                            <tr>
                                @can('update', $invoice)
                                    ...
                                    <td class="text-nowrap"><a
                                                href="{{ route('customers.edit', $invoice->customer->id) }}">{{ $invoice->customer->business_name }}</a>
                                    </td>
                                    ...
                                    <td class="d-flex justify-content-end">
                                        ...
                                        <button type="button" class="btn btn-link text-danger py-0 px-1"
                                                data-toggle="modal" onclick="handleDelete({{ $invoice->id }})">x
                                        </button>
                                    </td>
                                @else
            ...
            @else
                <p class="mb-0">You haven't created an invoice yet. <a href="{{ route('invoices.create') }}">Create
                        invoice</a></p>
            @endif
        </div>
    </div>
                ...
                <div class="modal-footer justify-content-center">
                    ...
                    <form action="" method="POST" class="d-inline" id="deleteForm">
                        ...
                        <button type="submit" class="btn btn-danger"
                                onsubmit="return confirm('Do you really want to submit the form?')">{{ __('Delete') }}</button>
                    </form>
                </div>
            </div>
        </div>
